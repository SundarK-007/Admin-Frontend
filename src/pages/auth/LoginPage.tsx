...
export default function LoginPage() {
  ...
  /**
   * Read in an effect, not a useState initializer. `takeSessionEndReason`
   * consumes the value, which makes it an impure initializer — and React
   * StrictMode calls those twice in development, so the first call swallowed
   * the reason and the second returned null. The `if (reason)` guard means
   * StrictMode's second effect pass leaves the already-set notice alone.
   */
  useEffect(() => {
  ...
  const { run, submitting, error } = useAsyncAction(async (values: FormValues) => {
    ...
    // /auth/login is shared with the future Customer Portal, so a devotee's
    // credentials are genuinely valid — they just don't grant Admin Panel
    // access. Say so here rather than storing the session and letting
    // ProtectedRoute bounce them with no explanation.
  ...
  return (
    <AuthShell
      ...
      footer={
        <p>
          Trouble signing in?{" "}
          <Link to="/forgot-password" className="text-amber-600 underline-offset-2 hover:underline">
            Reset your password
          </Link>
        </p>
      }
